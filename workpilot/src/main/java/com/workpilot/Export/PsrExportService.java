...
// Ajouter les bordures
            ...
            titleRow.setHeightInPoints(70); // Hauteur optimisée pour l'image
            //titleRow2.setHeightInPoints(20); // Ajuster la hauteur de la ligne 3 (pour le logo)

            // === LOGO en B2:D3 ===
            try (InputStream logoStream = getClass().getClassLoader().getResourceAsStream("static/img/logoTELNET.png")) {
                if (logoStream != null) {
                    ...
                    anchor.setRow1(1); // Ligne 2 (index 1)
                    anchor.setCol2(3); // Colonne D (pour plus de largeur)
                    anchor.setRow2(3); // Ligne 4 (index 3)
                    Picture pict = drawing.createPicture(anchor, pictureIdx);
                    pict.resize(0.6); // Réduire la taille pour un meilleur ajustement
                } else {
                    System.err.println("Image logoTELNET.png not found in resources");
                }
            } catch (IOException e) {
          ...
          /*  sheet.addMergedRegion(new CellRangeAddress(1, 2, 2, 3)); // C2:D3
            ...
            sheet.setColumnWidth(1, 25 * 256); // B (logo et labels) - largeur optimisée
...
//--------------------------------------------------------------------------------------------------
            ...
            // --- NOUVELLE LOGIQUE : Utiliser les TaskTracker comme dans le service WeeklyReportServiceImpl ---
            List<TaskTrackerDTO> taskTrackers = psr.getTaskTrackers();
            LocalDate psrReportDate = psr.getReportDate();
            int psrMonthValue = psrReportDate.getMonthValue();
            int psrYear = psrReportDate.getYear();
            String psrProjectName = psr.getProjectName();
            ...
            // Grouper les TaskTracker par semaine (comme dans WeeklyReportServiceImpl)
            Map<Integer, List<TaskTrackerDTO>> groupedTasksByWeek = new HashMap<>();
            if (taskTrackers != null) {
                for (TaskTrackerDTO task : taskTrackers) {
                    if (task.getWeek() != null) {
                        try {
                            // Extraire le numéro de semaine du format "2025-W22" ou "W22"
                            String weekStr = task.getWeek();
                            int weekNum;
                            if (weekStr.contains("-W")) {
                                weekNum = Integer.parseInt(weekStr.split("-W")[1]);
                            } else if (weekStr.startsWith("W")) {
                                weekNum = Integer.parseInt(weekStr.substring(1));
                            } else {
                                weekNum = Integer.parseInt(weekStr);
                            }
                            
                            groupedTasksByWeek.computeIfAbsent(weekNum, k -> new ArrayList<>()).add(task);
                        } catch (NumberFormatException e) {
                            // Ignorer les tâches avec un format de semaine invalide
                        }
                    }
                }
            }

            // Calculer les données agrégées pour le tableau "Monthly Workload By Project/Module"
            ...
            for (int weekNum : weeksInPsrMonth) {
                ...
                List<TaskTrackerDTO> tasksForThisWeek = groupedTasksByWeek.getOrDefault(weekNum, Collections.emptyList());
                
                // Calculer les totaux pour cette semaine (comme dans WeeklyReportServiceImpl.generateReportFromPsr)
                for (TaskTrackerDTO task : tasksForThisWeek) {
                    totalWorkingDaysForWeek += (task.getWorkedMD() != null ? task.getWorkedMD() : 0);
                    totalEstimatedDaysForWeek += (task.getEstimatedMD() != null ? task.getEstimatedMD() : 0);
                }
           ...
           // weeklySheet.addMergedRegion(new CellRangeAddress(totalsRowIndex, totalsRowIndex, 0, 1));
            ...
            // === ESPACE APRÈS LES TOTAUX ===
            weeklySheet.createRow(totalsRowIndex + 1); // Ligne vide
            weeklySheet.createRow(totalsRowIndex + 2); // Ligne vide
...
// Calculer chartDataEndRow
            int chartDataEndRow = workloadDataStartRow + monthlyWorkloadDataAggregated.size() - 1;

            if (chartDataEndRow >= workloadDataStartRow) { // S'assurer qu'il y a des données à afficher

                XSSFChart workloadChart = drawingWorkload.createChart(anchorWorkload);
                workloadChart.setTitleText("Charge de travail hebdomadaire");
                ...
                barChartDataWorkload.setBarDirection(BarDirection.COL); // Orientation des barres

                // Ajouter la première série (Jours travaillés)
                XDDFBarChartData.Series seriesWorkingDays = (XDDFBarChartData.Series) barChartDataWorkload.addSeries(categories, workingDays);
                seriesWorkingDays.setTitle("Jours travaillés (MD)", null); // Titre pour la légende
                ...
                // Ajouter la deuxième série (Jours estimés)
                ...
                seriesEstimatedDays.setTitle("Jours estimés (MD)", null); // Titre pour la légende
                ...
                // Assurer le regroupement des barres
                barChartDataWorkload.setBarGrouping(BarGrouping.CLUSTERED); // Barres côte à côte
                ...
                legendWorkload.setOverlay(false);
            }
