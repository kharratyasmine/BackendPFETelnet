...
public interface WeeklyReportRepository extends JpaRepository<WeeklyReport, Long> {
    ...
    @Query("SELECT w FROM WeeklyReport w WHERE w.psr.project.id = :projectId AND w.month = :month AND w.year = :year ORDER BY w.weekNumber ASC")
    List<WeeklyReport> findByProjectAndMonthAndYear(@Param("projectId") Long projectId, @Param("month") String month, @Param("year") int year);
